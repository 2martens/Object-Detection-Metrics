...
from lib.BoundingBoxes import BoundingBoxes, BoundingBox
from lib.Evaluator import Evaluator
from lib.utils import BBFormat, CoordinatesType


# Validate formats
def ValidateFormats(argFormat: str, argName: str, errors: List[str]) -> BBFormat:
    if argFormat == 'xywh':
...
def ValidateMandatoryArgs(arg, argName: str, errors: List[str]) -> bool:
    if arg is None:
...
def ValidateImageSize(arg, argName: str, argInformed: str, errors: List[str]) -> None:
...
def ValidateCoordinatesTypes(arg, argName: str, errors: List[str]) -> CoordinatesType:
    if arg == 'abs':
...
def ValidatePaths(arg, nameArg: str, errors: List[str]) -> Any:
    if arg is None:
...
def getBoundingBoxes(directory, isGT: bool, bbFormat: BBFormat, allBoundingBoxes: BoundingBoxes=None,
                     allClasses: List[str]=None) -> Tuple[BoundingBoxes, List[str]]:
...
parser = argparse.ArgumentParser(prog='Object Detection Metrics - Pascal VOC',
                                 description='This project applies the most popular metrics used to evaluate object '
...
# Mandatory
parser.add_argument('-gt', '--gtfolder', dest='gtFolder', metavar='',
                    help='folder containing your ground truth bounding boxes')
parser.add_argument('-det', '--detfolder', dest='detFolder', metavar='',
                    help='folder containing your detected bounding boxes')
# Optional
parser.add_argument('-t', '--threshold', dest='iouThreshold', type=float, default=0.5, metavar='',
                    help='IOU threshold. Default 0.5')
parser.add_argument('-gtformat', dest='gtFormat', metavar='',
                    help='format of the coordinates of the ground truth bounding boxes: (\'xywh\': <left> <top> '
                         '<width> <height>) or (\'xyrb\': <left> <top> <right> <bottom>)')
parser.add_argument('-detformat', dest='detFormat', metavar='',
                    help='format of the coordinates of the detected bounding boxes (\'xywh\': <left> <top> <width> '
                         '<height>) or (\'xyrb\': <left> <top> <right> <bottom>)')
parser.add_argument('-gtcoords', dest='gtCoordinates', metavar='',
                    help='reference of the ground truth bounding box coordinates: absolute values (\'abs\') or '
                         'relative to its image size (\'rel\')')
parser.add_argument('-detcoords', dest='detCoordinates', metavar='',
                    help='reference of the ground truth bounding box coordinates: absolute values (\'abs\') or '
                         'relative to its image size (\'rel\')')
parser.add_argument('-imgsize', dest='imgSize', metavar='',
...
for c in allClasses:
    ...
    metricsPerClass = evaluator.PlotPrecisionRecallCurve(c,  # Class to show
                                                         allBoundingBoxes,
                                                         # Object containing all bounding boxes (ground truths and
                                                         # detections)
