...
from .utils import BBType, BBFormat


class Evaluator:
    
    def GetPascalVOCMetrics(self, boundingboxes: BoundingBoxes, IOUThreshold: float = 0.5) -> list:
        ...
        for bb in boundingboxes.getBoundingBoxes():
            ...
            if bb.getBBType() == BBType.GroundTruth:
                ...
                detections.append([bb.getImageName(), bb.getClassId(), bb.getConfidence(),
    ...
    def PlotPrecisionRecallCurve(self, classId: str, boundingBoxes: BoundingBoxes, IOUThreshold: float = 0.5,
                                 showAP: bool = False, showInterpolatedPrecision: bool = False, savePath: str = None,
                                 showGraphic: bool = True) -> dict:
    ...
    @staticmethod
    def CalculateAveragePrecision(rec: List[float], prec: List[float]) -> list:
        mrec = [0]
    ...
    @staticmethod
    def _getAllIOUs(reference: BoundingBox, detections: List[BoundingBox]) -> \
            List[Tuple[float, BoundingBox, BoundingBox]]:
        ret = []
    ...
    def iou(boxA: Tuple[float, float, float, float], boxB: Tuple[float, float, float, float]) -> float:
        ...
        if not Evaluator._boxesIntersect(boxA, boxB):
            return 0
    ...
    @staticmethod
    def _boxesIntersect(boxA: Tuple[float, float, float, float], boxB: Tuple[float, float, float, float]) -> bool:
        if boxA[0] > boxB[2]:
    ...
    @staticmethod
    def _getIntersectionArea(boxA: Tuple[float, float, float, float], boxB: Tuple[float, float, float, float]) -> float:
    ...
    @staticmethod
    def _getUnionAreas(boxA: Tuple[float, float, float, float], boxB: Tuple[float, float, float, float],
                       interArea: float=None) -> float:
    ...
    @staticmethod
    def _getArea(box: Tuple[float, float, float, float]) -> float:
