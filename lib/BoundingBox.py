from .utils import BBFormat, BBType, CoordinatesType, convertToRelativeValues, convertToAbsoluteValues


class BoundingBox:
    def __init__(self, imageName: str, classId: str, x: float, y: float, w: float, h: float,
                 typeCoordinates: CoordinatesType = CoordinatesType.Absolute, imgSize: tuple = None,
                 bbType: BBType = BBType.GroundTruth, classConfidence: float = None, format: BBFormat = BBFormat.XYWH):
        """Constructor.
        Args:
    ...
    def getAbsoluteBoundingBox(self, format: BBFormat = BBFormat.XYWH):
        if format == BBFormat.XYWH:
            return (self._x, self._y, self._w, self._h)
        elif format == BBFormat.XYX2Y2:
            return (self._x, self._y, self._x2, self._y2)
    
    def getRelativeBoundingBox(self, imgSize: tuple = None):
    ...
    def getCoordinatesType(self) -> CoordinatesType:
        return self._typeCoordinates
    
    def getBBType(self) -> BBType:
        return self._bbType
    
    @staticmethod
    def compare(det1: 'BoundingBox', det2: 'BoundingBox') -> bool:
    ...
    @staticmethod
    def clone(boundingBox: 'BoundingBox') -> 'BoundingBox':
